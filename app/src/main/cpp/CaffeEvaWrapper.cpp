...
bool CaffeEvaWrapper::SetModel(const ENUM_CaffeModel& caffeModelSrc,
  ...
  case ENUM_CaffeModel::AlexNet:
    bmpImgIOPara.filePathMean =
        mainDirPath + "/storage/emulated/legacy/Q_CNN/AlexNet/imagenet_mean.single.bin";
    caffeModelName = "AlexNet";
    dirPathPara = mainDirPath + "/storage/emulated/legacy/Q_CNN/AlexNet/Bin.Files";
...
bool CaffeEvaWrapper::Proc(
  ...
  if (!rtnFlg) {  // failed
    errorMsg = "[CaffeEvaWrapper::Proc] could not open the BMP file";
    return false;
  }  // ENDIF: rtnFlg
